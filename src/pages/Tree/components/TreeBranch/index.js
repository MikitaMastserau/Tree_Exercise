      ...
      <div className={stylesForWrapper} style={branchShift}>
         <div className={styles.branch}>
            {branch.children.length > 0 && (
               <button className={styles.branch__openBtn} onClick={() => setIsOpen(!isOpen)}>{isOpen ? <i className="bi bi-caret-down-fill"></i> : <i className="bi bi-caret-right-fill"></i>}</button>
            )}
            <p className={styles.branch__text}>{branch.name}</p>
            <div className="btn-group btn-group-sm">
               <button className="btn btn-outline-secondary" onClick={() => handleAddBranch()}><i className="bi bi-plus"></i></button>
               <button className="btn btn-outline-secondary" onClick={() => handleDeleteBranch()}><i className="bi bi-dash"></i></button>
            </div>
         </div>
